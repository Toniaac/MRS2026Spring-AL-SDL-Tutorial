...
The optimizer, seeded with our manual data, converged on a very good solution by cycle 12 (optimization cycle 8), with average volume = 50.8 uL (target was 50.0) with a CV of 0.05% (2 replicates) and a time of 25s per run. Not bad for something that started at 15 uL! Note, the GUI graphs without the stars correspond to the last condition (not the top condition). The best parameters were:
